            ...
            <div class="col-md-12">
                <div class="bg-white p-5 rounded shadow article-container">
                    ...
                    <div class="d-flex justify-content-center align-items-center">
                        <img src="{{ asset(Storage::url($article->image)) }}" class="article-image img-fluid rounded mb-4"
                            alt="...">
                    </div>
                    ...
                    @if (Auth::user() && Auth::user()->is_revisor && !$article->is_accepted)
                        <div class="mt-5 article-details">
                            <a href="{{ route('revisor.acceptArticle', ['article' => $article->id]) }}"
                                class="btn-modify text-white my-2">Accetta articolo</a>
                            <a href="{{ route('revisor.rejectArticle', ['article' => $article->id]) }}"
                                class="btn-delete text-white my-2">Rifiuta articolo</a>
                        </div>
                    @endif
                            ...
                            <div class="card mb-3 border-light shadow-sm">
                                ...
                                <div class="card-footer bg-transparent border-top-0 text-muted">
                                    <div class="comment-actions d-flex justify-content-between align-items-center">
                                        <div class="mr-2">
                                            <span
                                                id="likes-count-{{ $comment->id }}">{{ $comment->likes->count() }}</span>
                                            <button class="like-comment-button btn btn-outline-primary btn-sm"
                        ...
                        <form class="comment-form" data-article-id="{{ $article->id }}">
                            ...
                            <div class="input-group">
                                ...
                                <div class="input-group-append col-4 p-0">
                                    <button type="submit" class="btn btn-primary rounded-left-0 w-100"
                                        style="background-color: #007bff; border-color: #007bff;">Invia</button>
                                </div>
                            </div>
                        </form>
                    </div>
